...
author:
  "Ismail Guennouni^1^, Quentin Huys^2^, Samuel Dupret^1^, Maarten Speekenbrink^1^"
...
A total of 318 participants were recruited on the Prolific Academic platform (prolific.co). The mean age of participants was `r round(mean(as.numeric(d_finished$exitSurvey.age), na.rm=TRUE),1)` years. Participants were paid a fixed fee of £5 plus a bonus payment dependent on their performance that averaged £0.71. The experiment had a 2 (Condition: Intervention or Control) by 3 (Game: Trust-Game Pre Intervention, Trust-Game Post Intervention, Prisoner's Dilemma Post Intervention) design, with repeated measures on the second factor. Participants were randomly assigned to one of the two levels of the first factor.
...
The strategy of the computerised investor was modelled on behaviour of human investors in a 10-round RTG [@king-casas_rupture_2008].  Using this data, we estimated a hidden Markov model (HMM) on investors' behaviour with three latent states, reflecting "low-trust", "medium-trust", or "high-trust". Each latent state was associated with a distribution over all possible investor actions (from 0 to 20 investment) that reflected the amount of trust (Figure \ref{fig:HMMPanels}.A). Over rounds, the investor can move between states, and the probability of these transitions was modelled as a function of the net return in the previous round (see Figure \ref{fig:HMMPanels}.B).
...
#   newCoax[[ns]] <- fit(newCoax[[ns]], emcontrol= em.control(maxit = 1000,random.start = FALSE))
# }
# 
...
# # Here we use parameters of fitted PrePost model as initial values of the FullContrast model to check whether we can approach true maximal likelihood. 
...
```{r table-BICs-coax, include =FALSE, warning = FALSE, echo=FALSE, ft.align="center", tab.playerId='table-BICs-coax', label='table-BICs-coax'}
...
coax_pre_plot <- ggplot(trans_prob,aes(x=investment)) + 
  ...
  labs(x = "Investment", y = "Transition probability", color='State Transitioned to', linetype = "Line Type") + 
