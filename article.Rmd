...
coax_pre_plot <- ggplot(trans_prob,aes(x=investment)) + 
  geom_line(aes(y=prob_coax, colour = as.factor(to), linetype="Post-intervention")) + 
  geom_line(aes(y=prob_pre, colour = as.factor(to), linetype="Pre-intervention")) +
  facet_wrap(~from) + 
  ylim(c(0,1)) + 
  ggtitle("Transition function pre- and post-intervention manipulation") + 
  labs(x = "Investment", y = "Transition probability", color='State Transitioned to', linetype = "Line Type") + 
  scale_linetype_manual(values = c("Post-intervention" = "solid", "Pre-intervention" = "dotted")) +
  theme_bw()
