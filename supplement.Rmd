...
We estimated a three-state model for investor's behaviour, using maximum likelihood estimation via the Expectation-Maximisation algorithm as implemented in the depmixS4 (Visser & Speekenbrink) package for R. The model was estimated using investments from existing datasets of human dyads playing 10 rounds of the RTG with the same trustee [@king-casas_rupture_2008; **VTC project dataset**]. The dataset consisted of a total of 381 games.
...
<!-- Figure \ref{fig:preDefectionModel} shows the result of fitting a mixed effect model to participants percentage returns in the RTG when we restrict the analysis to pre-defection trials. -->
...
```{r postDefectionModel,fig.cap = "Summary of results of a Mixed-effects model fitted to participants returns restricted to  post-defection trials", fig.align='center'}
...
<!-- Figure \ref{fig:postDefectionModel} shows the result of fitting a mixed effect model to participants percentage returns in the RTG when we restrict the analysis to post-defection trials.  -->
...
```{r table-HMMs,fig.cap = "Table comparing the HMM-full models fit assuming the latent number of states is between 2 and 7. Using BIC, the 6 state model fits best", out.width="70%", out.height="70%", fig.align='center'}
...
## Distribution of returns for each latent state for HMM model of participant’s returns

Focusing on the HMM-full models with 6 latent states (please see main manuscript for a definition) we estimated the investors' policy in each state assuming a discretised Gaussian response function as highlighted in the previous section (Figure \ref{fig:trusteeHMMPlot}). Each latent state was associated with a distribution over trustee actions. The latent states were ordered by the mean of the estimated Gaussian, such as higher numbered latent states reflect higher mean percentage return.


```{r trusteeHMMPlot,fig.cap = "Distribution of participants' percentage return for each of the latent states in the 6 state HMM-full model. The latent states are ordered by the mean of the Gaussian that best fits the policy in that state, so higher numbered states are more pro-social", fig.align='center'}
